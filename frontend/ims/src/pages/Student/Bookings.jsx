...
const Booking = () => {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        equipment_id: selectedEquipment.id, // Match the column name in the database
        start_time: new Date(bookingData.start_time).toISOString(), // Ensure ISO format
        end_time: new Date(bookingData.end_time).toISOString(), // Ensure ISO format
        user_id: parseInt(user_id, 10), // Ensure user_id is an integer
        purpose: bookingData.purpose || 'Not specified' // Optional field
      };
      ...
      console.log('Booking response:', response.data); // Debugging: Log the response
      ...
      console.error('Booking error:', err.response?.data || err.message); // Debugging: Log the error
      setBookingError(
        err.response?.data?.error || 
        'Failed to book equipment. Please try again.'
      );
